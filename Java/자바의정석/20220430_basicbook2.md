...
## 15-6 바이트 기반 스트림의 조상 - InputStream과 OutputStream  
### InputStream 메서드
![image](https://user-images.githubusercontent.com/101965836/167079526-a8ac81e0-527b-4a95-b798-b529a79cb0b4.png)   
    
### OutputStream 메서드    
![image](https://user-images.githubusercontent.com/101965836/167079777-db3e5ee1-aa99-43ef-8fa8-14dc574e9b82.png)   
   
   
## 15-7 InputStream과 OutputStream 예제 1

### read() 하면 byte array 하나씩 reduce 되는 느낌
**요약 : .read()를 하면 byte array의 0번 인덱스 요소부터 하나씩 읽으며, 커서를 한칸씩 옮기는 것처럼 동작한다고 보면 된다**   
  
여기서 어려운 부분은 while문이랑 input.read() 부분이다   
한번 read 하면 다음 요소로 넘어간다고 보면 된다  
예를 들어  
```java
...
```java 
System.out.println(input.read()); // 위 코드에다가 while 바로 앞에 .read() 하고 println하면? 
while((data = input.read())!=-1)
  output.write(data);	
```
이렇게 한번 .read() 하면   
...
```
이렇게 Outpust Source에는 제일 앞에 나오는 0이 빠지게 된다  
...
Output Source :[-1]
```
read를 다 하면 이렇게 -1이 된다  
  
  
## 15-8 InputStream과 OutputStream 예제 2
.read() .write() 에서 인자가 3개인 (byte\[] b, int off, int len)의 사용법에 대한 예제다  
b 배열을 앞에 off 만큼 제외하고 그 뒤로 len만큼 읽거나 쓴다.  
    
앞선 예제 1은 byte 하나씩 읽고 쓰지만,   
예제 2는 Array로 읽고 쓰기 때문에  
작업 효율이 더 좋다  
따라서 적절하게 배열을 사용하는 것이 좋다  

## 15-9 InputStream과 OutputStream 예제 3

## 15-10 FileInputStream과 FileOutputStream  
![image](https://user-images.githubusercontent.com/101965836/167087808-3dc8fe1b-452c-4148-8274-3b4bd967fecf.png)  
  
이외에도 오버라이딩된 몇 가지들이 더 있지만, 이 정도만 알아두면 된다    

## 15-11 FileInputStream과 FileOutputStream 예제 1
핵심 코드
```java
...
```
여기서   
int형으로 data 변수를 만들어서 .read()를 받는 이유는    
사실 .read() 하면 1 byte씩 들어오지만, 더 이상 읽을 데이터가 없을 때 -1을 반환하므로 -도 저장할 수 있는 int형을 쓴다   
또한 (char)data로 int형을 char형으로 변환하는데, 이것도 역시나 어차피 다 1byte 범위 안으로(0~255) 데이터가 들어오기 때문에  
int를 char로 변환해도 상관없다  
    
## 15-12 FileInputStream과 FileOutputStream 예제 2

## 15-13 FilterInputStream과 FilterOutputStream  
Filter 붙은 애들은   
InputStream OutputStream의 자손이자   
모든 바이트 기반 보조 스트림의 조상이다   
...
### FilterInputStream / FilterOutputStream은 자체로는 아무 일도 하지 않는다  
얘네들은 그냥 앞서 말했듯 InputStream과 OutputStream을 그대로 상속해서,  
그대로 메서드를 적어놨기 때문에  
Filter를 다시 상속해서 이 read()와 write() 메서드를 구현해줘야 한다  
...
> #### 왜 필요한가?   
> ![image](https://user-images.githubusercontent.com/101965836/167093736-cf327e3c-7404-46c3-8265-38c75b6e8dc7.png)  
...
생성자가 protected이기 때문에 FilterInputStream의 인스턴스를 생성해서 사용할 수 없고, 상속을 통해 오버라이딩 해줘야 한다  
  
### FilterI/OStream의 자손들  
...
## 15-14,15 BufferedInputStream  
![image](https://user-images.githubusercontent.com/101965836/167095454-2217c4c9-a8e6-4862-aab5-22be2ddf190a.png)  
   
입출력 효율을 높이기 위해 버퍼를 사용하는 보조스트림이다     
여러 바이트를 한번에 처리해서 효율이 좋기 때문에, 대부분의 입출력 작업에 사용된다.   
   
BufferedInputStream(InputStream in, int size) : in을 입력 소스로, size만큼 크기(byte단위)를 갖는 버퍼를 생성한다    
BufferedInputStream(InputStream in) : in을 입력 소스로, 기본적으로 8192 byte 크기(2\^13)의 버퍼를 갖는다    
    
input 버퍼크기는 입력소스로부터 한 번에 가져올 수 있는 데이터의 크기로 지정하면 좋다. 버퍼 크기를 변경해가면서 테스트하면 최적의 버퍼크기를 알아낼 수 있다.    
output도 버퍼를 이용하지만, 프로그램이 output버퍼에 데이터를 쌓고, 버퍼가 가득 차면 그때서야 버퍼의 모든 내용을 출력소스에 출력한다.    
그래서 마지막에 버퍼에 모든 값이 저장 안되면 파일에 출력되지 않고 끝날 수 있다. 그러므로 close()나 flush()를 호출해서 마지막에 버퍼에 쌓인 내용을 출력소스에 출력되도록 해야 한다.  

## 15-16 BufferedOutputStream 예제  
앞서 말한대로 끝에 .close() 하지 않고 끝나면 어떤 문제가 발생하는지 보여주는 예제이다  
또한, BufferedOutputStream의 close만 해줘도 FileOutputStream도 같이 close 되어서, 따로 호출해주지 않아도 된다.  

## 15-17 SequenceInputStream
여러 개의 입력스트림을 연속적으로 연결해서 하나처럼 쓸 수 있도록 해준다.  
생성자를 제외하고는 다른 입력스트림과 비슷하다    
작은 파일 여러개를 하나로 합칠 때 사용하면 적절하다  
![image](https://user-images.githubusercontent.com/101965836/167097528-40ba1c8d-90c0-4aa4-985b-c314111f990b.png)  

## 15-18 SequenceInputStream 예제  
vector에 입력된 순서대로 읽는다는 점.  
SequenceInputStream이 Enumeration을 사용하는 점(내부적으로도 Enumeration을 쓴다)  
정도 알아두면 된다   

## 15-19 PrintStream 
데이터를 기반 스트림에 다양한 형태로 출력할 수 있는 print, println, printf 같은 메서드를 오버로딩해서 제공한다   
JDK1.1부터는 printStream보다 향상된 문자기반 스트림인 PrintWriter가 추가되었으나, System.out이 PrintStream이다 보니까 둘 다 사용할 수 밖에 없게 되었다   
둘 다 거의 같은 기능을 하지만, 다양한 언어의 문자를 처리하는데에는 PrintWriter가 더 적합하다.  
따라서 기왕이면 PrintWriter를 사용하자  
  
![image](https://user-images.githubusercontent.com/101965836/167102062-3a314978-5d0f-4058-aa2d-88487d5e2fb7.png)
  
## 15-20,21 문자 기반 스트림 - Reader / Writer  
문자 기반 스트림에서는 Reader/Writer가 최고 조상 역할을 한다.     
다음은 Reader/Writer의 메서드인데, char배열을 사용한다는 것 외에는 바이트 기반 스트림의 메서드와 별반 다르지 않다    
![image](https://user-images.githubusercontent.com/101965836/167102470-15f5a6a5-9319-445a-8261-6c716810c8af.png)    
  
### 인코딩
문자 기반 스트림이라고 단순히 2 byte 단위로 스트림을 처리하지는 않는다.    
문자 데이터에는 인코딩(Encoding)도 필요하기 때문이다   
Reader/Writer는 여러 종류의 인코딩과 자바에서 사용하는 유니코드(UTF-16)간의 변환을 자동으로 처리해준다  

## 15-22 FielReader와 FileWriter  
파일로부터 텍스트 데이터를 읽고 쓰는데 사용한다.   
사용 방법은 FileInputStream/FileOutputStream과 다르지 않다.  
### 예제 15-8
각각 FielReader와 FileInputStream을 사용해서 같은 내용의 파일을 출력하는 예제 코드를 볼 수 있다.   
FileInputStream은 한글이 깨져서 출력된다.   
  
### 예제 15-9  
파일의 공백을 모두 없애는 예제이다   
  
## 15-23 StringReader와 StringWriter  
CharArrayReacer/CharArrayWriter와 같이 입출력 대상이 메모리인 스트림이다  
StringWriter에 출력되는 데이터는 내부에 있는 StringBuffer에 저장되며,  
...
String        toString()    
StringWriter에 있는 StringBuffer에 저장된 문자열을 반환한다    

### 예제 15-10
  
  
## 15-24 BufferedReader와 BufferedWriter    
byte형 stream에서 버퍼를 이용했던 것과 마찬가지로  
Reader/Writer를 위한 BufferedReader/BufferedWriter가 있다  
BufferedReader에는 readLine() , BufferedWriter에는 newLine()이라는 줄바꿈을 해주는 메서드를 갖고 있다    

### 예제 15-11  


## 15-25 InputStreamReader / OutputStreamWriter
바이트 기반 스트림을 문자 기반 스트림으로 연결시켜주는 역할을 한다   
![image](https://user-images.githubusercontent.com/101965836/167108631-b5299739-0dad-4d15-8f0a-5d67ded4d93c.png)  
  
### 주의할 점 
인코딩을 제대로 지정해줘야지 OS에서 제대로 해석할 수 있다  
인코딩을 지정하지 않으면 OS에서 사용하는 인코딩으로 데이터를 저장한다  

### 콘솔로 라인 단위 입력 받기 / 인코딩 변환 
![image](https://user-images.githubusercontent.com/101965836/167109403-9af27cea-3f0f-4995-b934-df84da5d3db3.png)  
![image](https://user-images.githubusercontent.com/101965836/167109425-fd87a37e-1254-4a4a-ba55-5fedd14d3042.png)  
  
BufferedReader의 readLine()을 이용해서 라인 단위로 입력받으면 편리하다    
하지만 JDK1.5부터는 Scanner가 추가되어, 이같은 방식을 사용하지 않아도 간단히 처리할 수 있다    
  
## 15-26 표준 입출력 (Standard I/O)  
콘솔을 통한 데이터 입력과 출력을 의미한다.    
자바에서는 표준 입출력(standard I/O)을 위해 3가지 입출력 스트림을 제공한다.    
System.in , System.out , System.err을 제공하는데,   
표준 입출력들은 자바 APP 실행과 동시에 사용할 수 있게 자동으로 생성되므로, 별도로 스트림을 생성할 필요가 없다   
    
![image](https://user-images.githubusercontent.com/101965836/167110185-399c764b-c02d-43f7-825e-79d1e26e0fd2.png)    
    
System클래스의 소스는 아래와 같다   
```
...
```
이게 선언 타입이 InputStream, PrintStream 이지만, 실제로는 BufferedInputStream과 BufferedOutputStream의 인스턴스를 사용한다  
  
## 15-27 표준 입출력의 대상변경  
표준 입출력 대상이 원래는 콘솔 화면이지만,  
setIn() setOut() setErr() 메서드를 사용하면 입출력을 콘솔 이외 다른 대상으로 변경할 수 있다  
![image](https://user-images.githubusercontent.com/101965836/167112140-bfe3bd3e-414f-4949-9e6c-519877d0af81.png)  
...
err : Hello World
```
out과 err 둘 다 출력대상이 콘솔이므로, 둘 다 똑같이 콘솔에 출력된다   
  
## 15-28 표준 입출력의 대상 변경 예제 
### 예제 15-14
System.out의 출력 대상을 test.txt 파일로 변경한 예제   


## 15-29 File
기본적이면서도 가장 많이 사용되는 File 형식의 입출력   
당연히 많이 사용되는만큼 중요하다  
File 클래스를 통해서 파일과 디렉토리를 다룰 수 있도록 설계되어 있다  
따라서 File 인스턴스는 "파일"일수도 있고 "디렉토리" 일수도 있다  

  
### 생성자 / 메서드
![image](https://user-images.githubusercontent.com/101965836/167112884-1ec275d6-5b5c-4e61-9185-157060ade85d.png)  
![image](https://user-images.githubusercontent.com/101965836/167113267-6e3629b5-ec37-466b-bcbb-a9f6ca15d223.png)  
  
### 멤버변수
![image](https://user-images.githubusercontent.com/101965836/167113092-3395b7cf-10b9-45e7-9f68-6e9fedae396d.png)  
  

## 15-30,31,32,33 예제1 / 예제2 / 예제3 / 예제4

### 예제1 - 파일 기본    
File f = new File() 생성 방법 및 기본 File 인스턴스의 디렉토리 관련 메서드들에 대한 예제이다       
또 **절대경로와 상대경로의 차이** 에 대해서     
절대경로는 중간에 '.'과 같은 기호나 링크를 포함할 수 있지만     
정규경로는 기호나 링크를 포함하지 않는 유일한 경로다   
```
절대경로 1      : c:\\jdk1.8\\work\\ch15\\Ex15_15.java
절대경로 2      : c:\\jdk1.8\\work\\.\\ch15\\Ex15_15.java
정규경로 (유일) : c:\\jdk1.8\\work\\ch15\\Ex15_15.java   
```
   
### 예제2 - 디렉토리 보여주기 
지정한 경로의 디렉토리를 보여주는 예제이다   
  
### 예제3 - 재귀 호출로 하위 디렉토리에 파일들 삭제
  
### 예제4 - renameTo 메서드 및 0000 파일명 앞에 붙여서 정렬  
  
  
## 15-34 직렬화(serialization)  
![image](https://user-images.githubusercontent.com/101965836/167143857-00e881fe-e42e-44b9-917e-37410fcae7dd.png)  
...
그냥 별 거 아니고 Object나 Data를 외부에서도 사용할 수 있도록 byte 형태로 만들어주는 작업을 말한다  
    
조금 자세히 객체에 대해 먼저 파고들어보면  
객체 안에는 오직 인스턴스 변수만 저장되어 있다. 당연하게도, 메서드는 어차피 클래스에 저장되어 있으니, 객체마다 또 메서드를 저장할 필요가 없다    
따라서 객체를 저장한다는 것은, 모든 인스턴스 변수의 값을 저장한다는 것이다.  
  
## 15-35 ObjectInputStream / ObjectOutputStream  
...
```
위 코드는 objectfile.ser 이라는 파일에 UserInfo 객체를 저장한다    
여기에 ObjectOutputStream.writeObject(Object obj)를 사용해 객체를 출력하면 파일로 저장된다.  
  
역직렬화도 마찬가지다.  
입력스트림을 사용해서 readObject()를 사용하여 저장된 데이터를 읽기만 하면 객체로 역직렬화 된다.   
```java
...
### 다양한 타입의 입출력 메서드들   
![image](https://user-images.githubusercontent.com/101965836/167143969-d439020f-069c-427d-9229-5405124c2a7b.png)  
  
![image](https://user-images.githubusercontent.com/101965836/167143948-e7a8aead-ab75-4828-83a9-29df76bbacb3.png)  
  
  
## 15-36 직렬화가 가능한 클래스 만들기
- 직렬화하고자 하는 클래스가 java.io.Serializable 인터페이스를 구현하도록 하면 된다  

### Serializable는 껍데기만 있는 인터페이스임
```
public interface Serializable {}
```
아무 메서드도 없으므로 그냥 implements만 해주면 바로 직렬화가 가능하다  
즉, Serializable 인터페이스는 아무 내용도 없는 빈 인터페이스이다. 그냥 Serializable은 "얘가 직렬화 가능합니다" 라고 알려주는 역할만 한다. 그냥 마킹역할만 한다.
   
또한 조상이 Serializable을 구현하면, 상속받은 자식도 역시 직렬화가 가능하다       
그러나 조상이 Serializable을 구현하지 않고 상속받은 자식이 구현했다면, 조상의 인스턴스 변수들은 직렬화 대상에서 제외된다.  
   
   
## 15-37 직렬화 대상에서 제외시키기 - transient  

### NotSerializableException 에러
직렬화 할 수 없는 클래스의 객체를 인스턴스변수가 참조하고 있으면   
NotSerializableException 에러가 발생한다.  

### 직렬화 가능 여부는, 참조 타입과 무관하고, 실제 객체가 뭔지에 달려있다  
#### (1) 참조타입 Object / 실제객체 Object - 직렬화X
```java
public class UserInfo implements Serializable {
  String  name;
  String  password;
  int     age;
  Object  obj = new Object(); // Object 객체는 직렬화 할 수 없다  
}
```

#### (2) 참조타입 Object / 실제객체 String - 직렬화O
```java
public class UserInfo implements Serializable {
  String  name;
  String  password;
  int     age;
  Object  obj = new String("abc"); // String은 직렬화 할 수 없다  
}
```
...
## 15-38,39,40 직렬화와 역직렬화 - 예제1 / 예제2 / 예제3  

### 예제1 - UserInfo 클래스 
뒤에 예제2 예제3을 위한 클래스이다  
예제2 예제3 실행 전에 컴파일하면 된다  
...
## 16-1 네트워킹이란?  
두 대 이상의 컴퓨터를 연결하는것  
최초에는 컴퓨터들끼리 서로 연결해서 데이터를 손쉽게 주고받고, 프린터나 주변기기 같은 자원들을 공유하고자 네트워킹이 시작됐다  
  
java.net 패키지를 사용하면, 네트워크 어플리케이션의 데이터 통신 부분을 쉽게 작성할 수 있다   
이번 16장에선 기본적인 네트워킹 예제와 채팅 앱을 작성할 정도의 수준만 다룬다  
...
### 서버기반모델 vs P2P모델  
서버기반모델(server-based model) : 네트워크 구성할 때 전용서버를 두는 것   
P2P모델(peer-to-peer model) : 별도의 전용서버없이 각 클라이언트가 서버역할을 동시에 수행   
  
![image](https://user-images.githubusercontent.com/101965836/167233065-70707c06-e0c7-4dce-9615-ae048885ecf6.png)  
  
  
## 16-3 IP주소 
IP주소는 4byte(32bit)의 정수로 구성되어 있음  (255.255.255.255)  
IPv4를 최초에 사용해왔으나 네트워크 기기의 증가에 따라 IP가 부족해지면서 IPv6를 도입했다.     
IPv6는 (2606:2800:0220:0001:0248:1893:25C8:1946) 같은 형식으로 16진수로 표기한다는 특징이 있다    
![image](https://user-images.githubusercontent.com/101965836/167233643-9952889d-4d16-4d75-8965-912e88eeea06.png)     
![image](https://user-images.githubusercontent.com/101965836/167233649-69ef614f-d5ad-48ed-aa8b-0af69f797834.png)    
우리나라의 IPv6 도입률은 보다시피 26.2% 이다. 생각보다 많네?    
[관련 통계는 여기서 볼 수 있다](https://www.akamai.com/ko/visualizations/visualizing-akamai)   
  
  
## 16-4 네트워크 주소와 호스트 주소  
IP주소와 서브넷 마스크로 네트워크 주소를 얻을 수 있다.  

### 일단 IP주소가 어떻게 이뤄져있는지 보자  
![image](https://user-images.githubusercontent.com/101965836/167233868-065eaf9f-0336-4029-8c41-0ce865b7c6b6.png)  
위의 그림은 IP주소를 2진수로 나타냈을 때 모습이다  
보다시피 IP주소는 앞 3묶음은 네트워크 주소이고, 마지막은 호스트 주소로 이뤄져 있다.  
네트워크 주소가 같으면 같은 네트워크에 존재한다는 뜻이다.  
  
### 서브넷 마스크란?  
앞에서 IP 주소가 네트워크 주소 부분과 호스트 주소 부분으로 나눠져 있다고 했다  
이에 따라서 간단하게 네트워크부분 ip 주소를 추출하기 위한 마스크(mask) 역할을 하는 것이 서브넷 마스크이다.  
먼저 마스크란 포토샵 같은 곳에서 사용하는 그 마스크다. 어떤 부분만 딱 추출해서 보여주기 위해 사용하는 그 마스크 말이다  

### 서브넷 마스크로 네트워크 부분 추출 - \&연산
서브넷 마스크도 마찬가지로 연산으로 딱 네트워크 부분만 을 추출할 수 있도록 하기 위한 마스크이다  
![image](https://user-images.githubusercontent.com/101965836/167234166-9f031149-827c-4b43-be99-270910543505.png)  
이렇게 & 연산을 통해 딱 네트워크 부분만 추출해낼 수 있다   

## 16-5 InetAddress 클래스  
자바에서 IP주소를 다루기 위한 클래스이다  
아래와 같은 메서드가 정의되어 있다  
![image](https://user-images.githubusercontent.com/101965836/167234317-66725bbc-7571-478d-a32c-7d6a01296ddd.png)  
     
#### 실행결과    
![image](https://user-images.githubusercontent.com/101965836/167234332-4c0aabbb-91e5-49e1-bf94-e499e0319402.png)    
    
    
## 16-6 InetAddress 클래스 예제  
앞선 InetAddress의 메서드들을 실행해보는 간단한 예제이다    
  
## 16-7 URL
Uniform Resource Locator  
인터넷에 존재하는 여러 서버들이 제공하는 자원에 접근할 수 있는 주소를 말한다  

### URL의 형식
```
프로토콜://호스트명:포트번호/경로명/파일명?쿼리스트링#참조
```
URL의 형식은 위와 같은 구조로 이뤄져 있다  

### 예시
```
http://www.codechobo.com:80/sample/hello.html?referer=codechobo#index1
```
프로토콜 : 서버와 통신하는데 사용되는 통신규약    
호스트명 : 서버의 이름    
포트번호 : 통신에 사용되는 포트. http 기본 포트는 80포트이므로 URL에서 포트번호를 생략하는 경우 80번 포트로 간주된다. 각 프로토콜에 따라 기본 포트가 다르며, 마찬가지로 포트번호가 생략되면 기본포트로 접근한다    
...
### URL 클래스의 메서드들  
![image](https://user-images.githubusercontent.com/101965836/167236148-25c2a933-3cd3-4ec1-80a2-070714edcf6b.png)  

## 16-9 URL클래스 예제  
URL 객체를 생성하고 간단히 앞선 메서드들의 기본적인 실행 결과에 대해 알아보는 예제이다   
  
  
## 16-10 URLConnection클래스  
앱과 URL간 통신 연결을 위한 추상 클래스   
![image](https://user-images.githubusercontent.com/101965836/167236519-c8f03788-c208-48eb-9ec7-ff8f35f23cc5.png)     
![image](https://user-images.githubusercontent.com/101965836/167236548-c2857a82-49be-47d1-b499-8d48d7608593.png)     
![image](https://user-images.githubusercontent.com/101965836/167236552-a868643a-592a-4d34-bbd4-f0cf1b24697f.png)     
다음 챕터 16-11의 예제를 참고해 보면 좋다  
URLConnection 클래스는 추상 클래스이므로, 다른 클래스를 통해 구현된 객체를 얻어와서 사용해야 한다  
예제의 경우 url 객체를 생성해서 url.openConnection()을 통해 구현된 객체를 얻는다  


## 16-11 URLConnection클래스 예제 1  
URLConnection이 구현된 클래스의 객체를 url.openConnection()을 통해 얻고 기본적인 사용 방법에 대해 알아보는 예제이다  

## 16-12 URLConnection클래스 예제 2   
url을 연결하여 그 내용을 읽어오는 예제이다  
URL이 유효하지 않으면 MalformedURLException이 발생한다  

## 16-13 URLConnection클래스 예제 3  
앞선 예제 2와 유사하지만, 텍스트 데이터가 아닌 이진 데이터를 읽어서 파일에 저장한다는 점만 다르다    

## 16-14 소캣(socket) 프로그래밍  
소켓이란 프로세스간의 통신에 사용되는 양쪽 끝단(endpoint)을 말한다  
마치 두 사람이 통신하기 위해 전화기가 필요한 것 처럼  
프로세스간 통신을 위해서 필요한 것이 소켓이다  
java.net 패키지를 통해서 소켓 프로그래밍을 지원한다  
  
TCP UDP를 이용한 소켓프로그래밍에 대해서 학습한다   

## 16-15 TCP와 UDP    
이기종 시스템간의 통신을 위한 프로토콜이다    
TCP와 UDP 모두 TCP/IP 프로토콜(TCP/IP protocol suites)에 포함되어 있으며    
OSI 7계층의 전송계층에 해당하는 프로토콜이다    
  
TCP와 UDP는 전송 방식이 다르며, 각 방식에 따른 장단점이 있다  
어플리케이션 특징에 따라 적절히 선택해서 사용하자   
![image](https://user-images.githubusercontent.com/101965836/167236970-edc6366a-12d5-4ae5-93ff-f5c82114f67d.png)  
  
UDP는 중간에 좀 데이터가 끊기더라도 빠른 전송이 필요한 경우에 사용된다. 게임이나 동영상 데이터 전송등이 있다  

## 16-16 TCP 소켓 프로그래밍  
TCP 소켓 프로그래밍은 클라이언트-서버 일대일 통신이다  

### 통신과정  
![image](https://user-images.githubusercontent.com/101965836/167237044-b1948a0d-c6fc-43f6-a96f-4e33a2bb06f8.png)   
  
여러 소켓이 하나의 포트를 공유해서 사용할 수 있지만, 서버 소켓은 다르다. 서버 소켓은 포트를 독점한다.    
또한 포트 번호는 0~65535의 범위에 속하는 값인데, 보통 1023번 이하 포트는 기존의 다른 통신 프로그램들(FTP, Telnet 등)에 의해 사용되는 경우가 많아서 1023번 이상의 포트를 사용해야 한다  


## 16-17 Socket과 ServerSocket  
서버 소켓은, 소켓간의 연결만 처리해주고, 실제 데이터는 소켓들끼리 주고받게 된다  
또한 각 프로세스간 입출력은 스트림으로 이뤄지며, 소켓은 입력 출력 스트림을 갖고있다  
당연히 한쪽의 입력 스트림은 반대쪽 출력 스트림과 연결되어있고, 이를 교차연결되어 있다고 표현하기도 한다   
![image](https://user-images.githubusercontent.com/101965836/167237283-a44a67b8-82ad-4706-9206-8216ff377e0d.png)  

### 자바의 소켓프로그래밍
자바에서는 TCP를 이용한 소켓프로그래밍을 위해 Socket과 ServerSocket 클래스를 제공한다    
![image](https://user-images.githubusercontent.com/101965836/167237299-bce00f44-a207-49a5-b2de-d5f941f4eb13.png)  
  
  
## 16-18,19 TCP소켓 프로그래밍 예제1 예제2
간단한 TCP IP 통신 예제이다  
예제1은 서버쪽 코드, 예제2는 클라이언트쪽 코드이다  

### 동작 방식  
본 예제에서는 간단히 자신의 컴퓨터에서 서버와 클라이언트 둘 다 실행하는 경우(127.0.0.1)로 코드를 작성했다  
하지만 실제로는 당연히 다른 컴퓨터 사이에 서로 연결하는 네트워킹 동작이 이뤄진다.  
따라서 아래의 동작방식 설명에서는 서버는 IP 192.168.10.100이라 두고, 클라이언트는 IP 192.168.10.101이라고 두겠다    

#### (1) 서버 프로그램을 실행한다  \> java TcpIpServer.java
#### (2) 서버소켓을 생성한다  
![image](https://user-images.githubusercontent.com/101965836/167237614-56b15de4-e4fc-4b66-b47d-7c3d31e56e0b.png)  
```
serverSocket = new ServerSocket(7777);  // TcpIpServer.java
```
  
#### (3) 서버소켓이 클라이언트의 연결 요청을 받기를 대기한다  
서버는 클라이언트의 접속 요청이 올 때까지 대기하고 있는다.  
```
Socket socket = serverSocket.accept(); // TcpIpServer.java
```

#### (4) 클라이언트가 서버소켓에 연결을 요청한다  
![image](https://user-images.githubusercontent.com/101965836/167237665-a586a1ab-0987-4fbf-a1c5-13683ea15dc7.png)     
클라이언트가 소켓을 생성하고, 서버에 "나 너랑 통신할려고 소켓 만들었으니, 너랑 연결할 소켓좀 줘"라고 요청한다  
```
// TcpIpClient.java
Socket socket = new Socket("192.168.10.100", 7777);
```
  
#### (5) 서버는 소켓을 생성하고 클라이언트랑 연결한다  
![image](https://user-images.githubusercontent.com/101965836/167237813-107206b3-34c2-452f-97c8-9abbb117eef6.png)  
![image](https://user-images.githubusercontent.com/101965836/167237818-028d91c8-0362-4820-b416-ca53263675e2.png)  


## 16-20,21 UDP 소켓 프로그래밍 - Client / Server
TCP에서는 Socket과 ServerSocket을 사용하지만    
UDP에서는 DatagramScoekt과 DatagramPacket을 사용한다  
  
### TCP와 UDP의 차이  
앞서 말했듯, UDP는 클라이언트가 제대로 데이터를 받았건 말건 계속 그냥 와장창 보내는 방식이다  
따라서 TCP같이 연결지향적인 프로토콜이 아니므로 ServerSocket이 필요하지 않다  
  
  
###  DatagramScoekt과 DatagramPacket
각각 소켓과 데이터를 담을 곳이다    
DatagramPacket은 헤더와 데이터로 구성된다   
헤더에는 수신할 호스트의 주소와 포트가 저장된다  
    
packet이라는 이름 답게, 소포처럼 그냥 수신할 호스트의 주소와 포트를 적고 데이터를 담아 보내면  
대충 어쩌고 저쩌고 알아서 지정된 주소의 DatagramSocket에 도착하게 된다   
  
### 예제 코드 UdpClient.java / UdpServer.java  
간단히 서버의 시간을 받아 출력하는 프로그램이다  
